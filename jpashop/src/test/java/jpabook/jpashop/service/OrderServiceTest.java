...
public class OrderServiceTest extends TestCase {
    ...
    public void 상품주문() throws Exception {
        ...
        Assert.assertEquals("주문한 상품의 개수가 같아야 한다.", 1, getOrder.getOrderItems().size());
        Assert.assertEquals("주문 가격은 가격 * 수량이다.", 10000 * 2, getOrder.getTotalPrice());
        Assert.assertEquals("주문 수량 만큼 재고가 감소되어야 한다.", 8, book.getStockQuantity());
    ...
    private Member createMember() {
        ...
        member.setName("choi");
        member.setAddress(new Address("서울", "1번가", "123-123"));
        em.persist(member);
        return member;
    }
